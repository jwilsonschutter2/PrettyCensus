...
The site now has a fixed-header theme switch. Light uses the earlier grey and blue palette. Dark uses black, charcoal, grey, and red, closely following the attached stylesheet's Work Sans body type, Oswald headings, compact controls, dark panels, red selected state, and two-pixel shadow treatment. The browser stores the selected theme in localStorage and initially follows the operating-system preference when no saved choice exists.
